...
const meta = { title: 'Formularios/Campos', parameters: { layout: 'padded' } } satisfies Meta
export default meta
  ...
  render: () => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 340 }}>
        ...
        <MoneyInput label="Con pista" currency="USD" value="1.450" onChange={() => {}} hint="≈ $5.771.725" />
        <MoneyInput label="Con error" currency="COP" value="0" onChange={() => {}} error="Debe ser mayor que cero" />
        <MoneyInput label="Sin id explícito" currency="COP" value="120.000" onChange={() => {}} />
      </div>
    )
  },
}

export const FechaYSeleccion: Story = {
  name: 'Fecha y selección',
  render: () => {
    const [d, setD] = useState('2026-08-19')
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 340 }}>
        <Field label="Fecha de ingreso">{id => <DatePicker id={id} value={d} onChange={setD} />}</Field>
        <Field label="Sin fecha">{id => <DatePicker id={id} value="" onChange={() => {}} />}</Field>
        <Field label="Cuenta">
          {id => (
